 ...
 * ## Создание контактов
 *
 * Контакты создаются как локальные (пустые `account_type` / `account_name`).
 * На устройствах, где облачный аккаунт является единственным допустимым хранилищем,
 * создание локального контакта может завершиться ошибкой — в таком случае метод
 * [addContact] вернёт [Result.failure] с понятным сообщением об ограничениях.
 *
 * ## Потокобезопасность
 *
...
interface ContactsClient {
    ...
    /**
     * Добавить новый локальный контакт на устройство.
     *
     * Создаёт raw_contact с пустыми `account_type` и `account_name` (локальный контакт),
     * затем вставляет data-строки (имя, телефоны, email и т.д.).
     *
     * Если устройство запрещает создание локальных контактов, возвращает [Result.failure]
     * с объяснением ограничений Android Contacts Provider.
     *
     * @param deviceId ID устройства.
     ...
     *
     * Вызывает [addContact] для каждого элемента. При ошибке на одном контакте
     * продолжает импорт остальных. Возвращает итоговую статистику в [ImportResult].
     *
     * @param deviceId ID устройства.
