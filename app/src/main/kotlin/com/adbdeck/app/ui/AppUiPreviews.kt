...
private class PreviewRootComponent(
    ...
    private fun childFor(screen: Screen): RootComponent.Child = when (screen) {
        is Screen.Dashboard -> RootComponent.Child.Dashboard(dashboard)
        is Screen.Devices -> RootComponent.Child.Devices(devices)
        is Screen.Logcat -> RootComponent.Child.Logcat(logcat)
        is Screen.Settings -> RootComponent.Child.Settings(settings)
        is Screen.Packages -> RootComponent.Child.Packages(packages)
        is Screen.SystemMonitor -> RootComponent.Child.SystemMonitor(systemMonitor)
        is Screen.FileSystem -> RootComponent.Child.FileSystem(fileSystem)
        is Screen.FileExplorer -> RootComponent.Child.FileExplorer(fileExplorer)
        is Screen.Contacts -> RootComponent.Child.Contacts(contacts)
        is Screen.ScreenTools -> RootComponent.Child.ScreenTools(screenTools)
        is Screen.ApkInstall -> RootComponent.Child.ApkInstall(apkInstall)
        is Screen.DeepLinks -> RootComponent.Child.DeepLinks(deepLinks)
        is Screen.Notifications -> RootComponent.Child.Notifications(notifications)
        is Screen.DeviceInfo -> RootComponent.Child.DeviceInfo(deviceInfo)
        is Screen.QuickToggles -> RootComponent.Child.QuickToggles(quickToggles)
        is Screen.Scrcpy -> RootComponent.Child.Scrcpy(scrcpy)
    }
}
