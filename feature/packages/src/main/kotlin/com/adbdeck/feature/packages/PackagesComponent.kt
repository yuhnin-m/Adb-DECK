...
interface PackagesComponent {
    ...
    /**
     * Перейти на экран Logcat и подставить имя пакета в фильтр.
     *
     * Используется как быстрый переход из детальной панели пакета
     * к анализу логов конкретного приложения.
     *
     * @param pkg Пакет, который нужно отследить в Logcat.
     */
    fun onTrackInLogcat(pkg: AppPackage)

    /**
     * Выгрузить пакет на локальный хост с авто-детектом формата.
     *
     * Компонент определяет тип пакета по числу APK-путей:
     * - обычный пакет -> выгружает `.apk`
     * - split-пакет -> выгружает `.apks` архив
     *
     * Путь выбирается в UI через диалог сохранения и передается в компонент.
     * Расширение при необходимости корректируется автоматически.
     *
     * @param pkg       Пакет для выгрузки.
     * @param localPath Абсолютный путь файла назначения на хосте.
     */
    fun onExportApk(pkg: AppPackage, localPath: String)
